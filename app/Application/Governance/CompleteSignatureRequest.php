...
final class CompleteSignatureRequest
{
    ...
    public function execute(
        ...
        return DB::transaction(function () use ($user, $business, $requestId): ?SignatureRequest {
            ...
            if ($request === null || $request->status !== SignatureRequestStatus::FullySigned) {
                return null;
            }
            ...
            $request->fill([
                'status' => SignatureRequestStatus::Completed->value,
                'completed_at' => now(),
            ])->save();
            ...
            $this->occurrence->record(
                $user,
                $business,
                'governance.signature_request.completed',
                'signature_request',
                (string) $request->getKey(),
                [],
